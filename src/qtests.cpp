...
{
    std::string test_name = "Testing Q::Q( const Z& num_, const Z& den_ ) and Q::toString( int decimal_places ).";
    bool pass = true;
    ...
    for( int i = 1; i <= 100; i++ )
    {
        ...
        /* Testing that toString( int ) performs rounding of numbers correctly. */
...
{
    std::string test_name = "Testing Q::Q( const std::string& s ) and Q::toString( int n ).";
    bool pass = true;
...
{
    std::string test_name = "Testing Q::operator==( const Q& rhs ).";
    bool pass = true;
...
std::pair<std::string, bool> qTestCauchy_01()
{
    ...
    for( int d = 1; d <= 20; d++ )
    {
        /* We will use vector x = {1, 2, 3,..., d} to construct a Cauchy matrix of order d. */
        ...
        for( int i = 0; i < d; i++ )
            ...
            for( int j = 0; j < d; j++ )
            {
                ...
                /* Computing coefficients in an alternative way should yield the same result. */

                auto e = zqnum::cauchyMatrixInvCoef( x, x, i, j );
...
std::pair<std::string, bool> qTestCauchy_02()
{
    ...
    for( int d = 1; d <= 8; d++ )
    {
        ...
        /* Computing coefficients in two different ways should yield the same result. */
